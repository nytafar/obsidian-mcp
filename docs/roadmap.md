...
| 0.3 | **Norwegian keyword fix** — tsvector `'english'`→`'simple'` (or per-note lang config) | Recovers NO keyword recall | trivial |
...
| 2.2 | **Richer frontmatter filters** (C1) — ranges, `IN`, exists, negation, `since`/`until` | The Bases-equivalent query layer (see roadmap note) | med |
...
| # | Item | Why | Effort |
|---|------|-----|--------|
| 3.1 | **Server-side schema engine** (E1, C3) — kind registry, `scaffold_note`, write validation, `lint_vault` | Turns MVP into a structured ops layer; fixes drift | med-high |
...
| 3.4 | **Composable agent toolset** — `follow-links`, frontmatter-filtered list, optional server-side `grep`, `suggest_links` (E5), daily-append (E2), task primitives (E3) | Where agentic retrieval lives | med |
...
|---|------|-------------------|
| 4.1 | Metadata-feature reranker (maturity/recency/kind; heuristic→LambdaMART) | eval shows per-signal nDCG lift |
...
| 4.6 | Chunk overlap A/B (0 → 10–20%) | eval A/B |

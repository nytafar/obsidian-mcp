...
async def _log_usage(tool: str, params: dict, duration_ms: int, response_size: int) -> None:
    """Write a usage row via the centralized, correlation-aware logger.

    Kept as a module-level hook (tests stub this symbol directly) that delegates
    to `usage.log_usage`, which owns the DB write and correlation-id handling.
    """
    await usage.log_usage(tool, params, duration_ms, response_size)
